            ...
            <Card.Content>
                <div class="space-y-4">
                    {#each category.questions as question, index}
                        <div class="rounded-lg border p-4 {question.selectedScore === null ? 'bg-muted/30' : ''}">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="text-sm font-medium text-muted-foreground">Frage {index + 1}</span>
                                        {#if question.selectedScore !== null}
                                            <span class="text-xs px-2 py-1 rounded-full bg-muted {getScoreColor(question.selectedScore)}">
                                    ...
                                    {#if question.selectedScore !== null}
                                        <div class="mb-3">
                                            <p class="text-sm text-muted-foreground mb-1">Deine Antwort:</p>
                                            <p class="text-sm font-medium text-foreground bg-muted px-3 py-2 rounded">
                                                {getSelectedAnswer(question)}
                                            </p>
                                        </div>
                                    {:else}
                                        <p class="text-sm text-muted-foreground italic">Diese Frage wurde noch nicht beantwortet.</p>
                                    {/if}

                                    <div class="mt-3">
                                        <p class="text-xs text-muted-foreground mb-2">Alle Antwortmöglichkeiten:</p>
                                        <div class="space-y-1">
                                            {#each question.options as option, optionIndex}
                                                <div class="flex items-center gap-2 text-xs">
                                                    <span class="w-4 h-4 rounded-full border flex items-center justify-center {question.selectedScore === optionIndex + 1 ? 'bg-primary text-primary-foreground border-primary' : 'bg-muted border-muted-foreground'}">
                                                        {#if question.selectedScore === optionIndex + 1}
                                                            ✓
                                                        {/if}
                                                    </span>
                                                    <span class="{question.selectedScore === optionIndex + 1 ? 'font-medium' : 'text-muted-foreground'}">
                                                        {option}
                                                    </span>
                                                </div>
                                            {/each}
